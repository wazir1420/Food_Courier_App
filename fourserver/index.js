...
const handleMessage = (socket, message) => {
  ...
  switch (true) {
    ...
    case messageStr.startsWith("/help"):
      socket.emit(
        "message",
        "Available commands: /stats, /time, /date, /day, /weather, /joke, /roll, /flip, /about"
      );
      break;
    ...
    case messageStr.startsWith("/stats"):
      ...
      socket.emit(
        "message",
        `Active users: ${activeUsers}, Uptime: ${uptime.toFixed(2)} seconds.`
      );
      break;
    ...
    case messageStr.startsWith("/roll"):
      socket.emit("message", `You rolled a ${rollDice()}`);
      break;

    case messageStr.startsWith("/flip"):
      socket.emit("message", `The coin landed on ${flipCoin()}`);
      break;
